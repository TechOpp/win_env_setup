...
:CASE_15
  COLOR 1B
  set JAVA_HOME=D:\Java\JDK\jdk15_64bit
  GOTO END_CASE
:CASE_16
  COLOR 1B
  set JAVA_HOME=D:\Java\JDK\jdk16_64bit
  GOTO END_CASE  
:DEFAULT_CASE
  cls
...
:END_CASE
 ...
 rem On second run remaining part should not reset
	if not defined SunEnv (
					...
					echo Now JAVA %color% is Activated
					goto path
				)
 ...
 rem ENV var to get python path for google cloud SDK should not in path. this is only used by google cloud shell
  ...
  set tools=D:\Java\maven\bin;D:\Java\ant\bin;D:\WORK\NASM;D:\WORK\MinGW\mingw64\bin;D:\TOOLS\Git\cmd;D:\TOOLS\gawk\bin;D:\TOOLS\cURL\bin;D:\TOOLS\ffmpeg;D:\TOOLS\OpenSSL\bin;D:\TOOLS\Extra;D:\TOOLS\openshift-CLI;D:\TOOLS\putty;D:\WEB_SERVERS\nodejs\Yarn\bin;D:\Java\dex2jar-2.0;D:\WEB_SERVERS\SQLite;D:\WEB_SERVERS\Redis;D:\WEB_SERVERS\memcached;D:\TOOLS\mongodb\bin;D:\TOOLS\cmake\bin;D:\TOOLS\cdrtools;D:\VERTUAL_OS\qemu;D:\TOOLS\kubernete-CLI
 ...
 rem PYTHONPATH env var is for looking python module path 
  ...
  set RLang=D:\WEB_SERVERS\DSci-RLan\ActiveR\bin
  set GOPATH=D:\WEB_SERVERS\Go\ActiveGo
rem  set classpath=.
 ...
 rem R stands for hibernate library
 ...
 rem A stands for Android library. This is not full android library 
 ...
 rem setting environment for building Hybrid android application using Gradle tool
  ...
  rem NOTE: jme_lib + selenium_lib + android, these 3 library make jdk 0 to 2 not working when setting in classpath
rem  set classpath=%classpath%;%default_classpath%%default_jar%%jvm_state%%spring%%hibernate%%jme_lib%%selenium_lib%%android%
  ...
  ECHO Now, Java %COLOR%, GCC, JME, JVM Monitor, Maven, Ant, MinGW64, Gradle, Selenium, php, nasm, Spring, Hibernate, openssl, Nodejs, python, Ruby, Putty, SQLite, Redis, Memcache, IONIC, Cordova, 4 variants of C, CPP are availables... rem '&' will ignore to printing remaining part of echo
  ...
  set path=%default_path%;%java_path%;%tools%;%jvmstat_home%\bat;%servers%;%jme_path%;%NODE_PATH%;%PHP_PATH%;%PYTHONPATH%;%Clang%;%Hybrid%;%CrossBrowserAppTest%;%IDE%;%ruby%;%RLang%;%GOPATH%\bin
